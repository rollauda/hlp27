# Mémo — publier le site HLP

Site à **deux niveaux** dans un seul dépôt : Première (`premiere/`) et Terminale (`terminale/`).
...
**Modifier une page de leçon ici** — `premiere/<NN-slug>/` et `terminale/<NN-slug>/` sont **générés** depuis le coffre. Une correction faite dans ce dépôt sera écrasée à la prochaine publication. Corriger la source dans `prepa-hlp/`, puis republier.
...
Utile depuis une machine sans l'environnement installé — un poste du lycée, une tablette. Depuis GitHub : **Code › Codespaces › Create codespace**. Le dépôt s'ouvre dans VS Code en ligne, le port 3000 est exposé automatiquement.
